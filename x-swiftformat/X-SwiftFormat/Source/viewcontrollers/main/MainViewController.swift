...
class MainViewController: NSViewController {
  ...
  override func viewDidAppear() {
    ...
    if SponsorsViewController.showSponsorWindow() {
      self.performSegue(
        withIdentifier: NSStoryboardSegue.Identifier("MAIN_TO_SPONSORS"), sender: nil)
    }
  }
  ...
  private func checkUpdates() {
    ...
    let task = URLSession.shared.dataTask(with: request) { data, _, _ in
      ...
      self.validateNewVersion(
        name: name, tagName: tagName, publishedAt: publishedAt,
        browserDownloadURL: browserDownloadURL)
    }
    task.resume()
  }

  private func validateNewVersion(
    name: String, tagName: String, publishedAt: String, browserDownloadURL: String
  ) {
  ...
  func reloadData() {
    DispatchQueue.main.async {
      ...
      self.environmentRemove.isEnabled =
        (self.sharedConfiguration.environment != kNoEnvironmentLabel)
    ...
    DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
      ...
      alert.beginSheetModal(for: window) { response in
        if response.rawValue == 1000 && environmentName.stringValue.count > 0 {
          self.sharedConfiguration.setConfiguration(
            configuration: payload, for: environmentName.stringValue)
        }
      }
    }
  }
